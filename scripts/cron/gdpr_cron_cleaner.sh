...
find "$CRON_DIR" -path "$CRON_DIR/conf" -prune -o -type f -mtime +"$EXPIRE_DAYS" -printf '%TY-%Tm-%Td %TH:%TM:%TS %p\n' | sort
...
#find "$CRON_DIR" -path "$CRON_DIR/conf" -prune -o -type f -mtime +"$EXPIRE_DAYS" -print0 | xargs -0 --no-run-if-empty rm -v --
