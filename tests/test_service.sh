...
apache_service=$(testResponse "service apache2 status") 2> /dev/nul 
valid_status=("running" "dead")
#echo "Checking Apache2.4 status: "
apache_running=$(testResponse "service apache2 status" "${valid_status[@]}" || echo "")
if [ -z "$apache_running" ]; then
	...
	outputHandler "comment" "Apache Installed" "[Apache status:] $(testResponse "service apache2 status" "${valid_status[@]}")"
fi