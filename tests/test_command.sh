...
echo "Testing testResponse"
echo 
valid_status=("running" "dead")
echo "Checking Apache2.4 status: "
testResponse "service apache2 status" "${valid_status[@]}"
echo
echo "Checking Redis status: "
testResponse "service redis status" "${valid_status[@]}"
echo
echo "Checking unzip version: "
valid_version=("^UnZip ([6\.[0-9])")
testResponse "unzip -v" "${valid_version[@]}"
